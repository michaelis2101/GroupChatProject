...
const GroupChatScreen = ({navigation}) => {
    ...
    const { groupName, members, roomId } = route.params;
    ...
    const [id_Rooom, setId_Room] = useState(roomId);
    ...
    useEffect(() => {
    const groupMessagesRef = firebase
      .firestore()
    ...
    function handleSnapshot(snapshot) {
      ...
      snapshot.forEach(doc => {
        const { text, user, createdAt, roomId, attachmentUrl, attachmentName } = doc.data();
        if (
          (id_Rooom === roomId)
        ) {
          groupMessages.push({
    ...
    if (fileUri) {
      ...
      try {
        ...
        const groupMessagesRef = firebase.firestore().collection('groupMessages');
...
const renderItem = ({ item }) => {
    const isSentByUser = item.user === idUser;
    const isImageAttachment = /\.(gif|jpg|jpeg|png)$/i.test(item.attachmentUrl);
    const senderUsername = members.find(member => member.userId === item.user)?.username;
    ...
    const usernameTextStyle = [
      styles.usernameText,
      isSentByUser && styles.sentUsernameText,
    ];

    return (
      <View style={messageContainerStyle}>
        <Text style={usernameTextStyle}>{senderUsername}</Text>
        {item.attachmentUrl && (
          <TouchableOpacity
    ...
    return(
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.name}>{groupName} ({participants.length})</Text>
      </View>
      <FlatList
        data={messages}
